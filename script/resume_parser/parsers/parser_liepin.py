...
def parse(filename, filetext, fileori):
    ...
    # 处理猎聘开头出现 \- 的情况
    filetext = "\n".join( map(lambda x: re.sub("\\\\-","",x).strip(), filetext.split("\n")))
    ...
    for template in templates.get_parse_templates(pname):
        ...
        if len(resume_ret["basic"]["name"]) < 1:
            logger.info("parser:%s miss all basic_name . using time:%f" % (pname+template.template_name,  time.time()-start))
            continue
